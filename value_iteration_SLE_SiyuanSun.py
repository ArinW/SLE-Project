...
for s in range(num_states):
    ...
    # Skip states that are outside specified ranges
    if not (1 <= theta_index <= 6 and 1 <= x_index <= 3 and
            1 <= theta_dot_index <= 3 and 1 <= x_dot_index <= 3):
        continue

    # Initialize an array to store the calculated values for actions
    new_action_values = np.zeros((2, 1))

    # Calculate the value for the first action
    new_value = sum(trans_prob[s, i + 1, 0] * (1 + value_function[i + 1]) for i in range(num_states - 1))
    new_action_values[0] = new_value

    # Calculate the value for the second action
    new_value = sum(trans_prob[s, i + 1, 1] * (1 + value_function[i + 1]) for i in range(num_states - 1))
    new_action_values[1] = new_value

    # Update the policy with the action that has the maximum value
...
x_dot_value = 0  # Example value, here we didnt specify high, mid and low as what we did for theta_dot since we are only caring the mid value
...
ax.set_xlabel(r'Pole Angle $\theta$ deg', fontsize=labelsize)
ax.set_ylabel('Positionof Moving Cart  x (meters)', fontsize=labelsize)
